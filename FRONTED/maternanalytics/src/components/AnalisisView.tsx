...
// `AnalisisCompleto` es una unión discriminada por `tipo` (mortalidad |
// morbilidad); en tiempo de ejecución el backend solo envía las claves
// correspondientes a cada tipo. El código original (JS, sin tipos) accede a
// estas claves usando comprobaciones "truthy" (`data.demoras && ...`) en
// lugar de narrowing por `data.tipo`, así que aquí se amplía la forma con
// todas las claves opcionales para reflejar fielmente ese acceso sin
// restructurar la lógica.
// `estadisticas_basicas` está tipado como `Record<string, unknown>` en
// types.ts (Task 3) porque su forma exacta depende del backend. El código
// original accede a sus campos numéricos directamente (`.toFixed(1)`, etc.)
// sin narrowing; esta forma local documenta esos campos para el acceso de
// solo lectura que hace este archivo, sin tocar el tipo compartido.
interface EstadisticasBasicas {
  total_casos: number
  gestaciones_promedio?: number
  controles_prenatales_promedio?: number
  estancia_hospitalaria_promedio?: number
}

type AnalisisData = AnalisisCompleto & {
...
const BARRERAS_LABELS: Record<string, string> = {
...
const CIE10_DESCRIPTIONS: Record<string, string> = {
...
// Nota: el .jsx original también definía `tokenizeHeader` y
// `headerMatchesPattern`, pero ninguna función del componente las invocaba
// (verificado con grep sobre todo el archivo) — era código muerto. El
// tsconfig de este proyecto tiene `noUnusedLocals: true`, que convierte el
// código muerto en un error de compilación, así que se eliminó junto con su
// único llamante interno (`tokenizeHeader`); no cambia ningún comportamiento
// en tiempo de ejecución.

function normalizeCie10(code: unknown) {
  const raw = (code ?? '').toString().trim().toUpperCase()
  return raw.replace(/\s+/g, '')
}

function getCie10Description(code: string) {
...
function formatCie10Label(code: unknown) {
  const normalized = normalizeCie10(code)
  const desc = getCie10Description(normalized)
  return normalized ? `${normalized} - ${desc}` : `Sin código - ${desc}`
}

function buildPercentLabels(values: unknown[], totalOverride?: unknown) {
...
function getClusterColor(clusterId: unknown) {
...
function getReadableFeatureName(featureName: string) {
...
function buildClusterDescriptor(profile: ClusterProfile | undefined, allProfiles: ClusterProfile[]) {
...
function AnalisisView({ analisisId, onBack, showBackButton = true, embedded = false, filterYear = '', filterMonth = '', onAvailableYears = null }: AnalisisViewProps) {
  ...
  const [clusteringData, setClusteringData] = useState<Record<string, unknown> | null>(null)
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      try {
        ...
        if (isMounted) {
          setError((err as Error)?.message || 'Error al cargar análisis')
          setData(null)
        }
      } finally {
        ...
        <div className="header-info">
          ...
          {(filterYear || filterMonth) && (
            <div className="filter-active-badge">
              ...
              <span className="filter-active-count">{(data.estadisticas_basicas as unknown as EstadisticasBasicas | undefined)?.total_casos ?? 0} casos</span>
            </div>
          )}
        </div>
      </div>
...
interface InstitucionReferencia {
...
interface ObstetricoVariable {
  nombre: string
  por_edad: Record<string, number[]>
  valores_eje: (string | number)[]
  conteos_total: number[]
  promedio: number
}

interface ExtraChartData {
  title: string
  subtitle: string
  values: number[]
  labels: string[]
...
function ChartsTab({ data, analisisId }: ChartsTabProps) {
  ...
  const [extraChart, setExtraChart] = useState<ExtraChartData | null>(null)
  const [extraChartLoading, setExtraChartLoading] = useState(false)
  const [extraChartError, setExtraChartError] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    const cargarHeatmap = async () => {
      ...
      try {
        ...
        if ((err as Error)?.name === 'AbortError') return
        if (isMounted) {
          setHeatmapError((err as Error)?.message || 'Error al cargar heatmap.')
          setHeatmapData(null)
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const cargarExtraChart = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setExtraChart(null)
          setExtraChartError((err as Error)?.message || 'No se pudo generar el gráfico adicional.')
        }
      } finally {
  ...
  const getInstitucionChart = () => {
    const inst = data.institucion_referencia as InstitucionReferencia | undefined
  ...
  const getTiempoRemisionChart = () => {
    const tr = data.tiempo_remision as TiempoRemision | undefined
  ...
  const getObstetricoEdadChart = () => {
    const obs = data.obstetrico_edad as Record<string, ObstetricoVariable> | undefined
...
interface ClusteringTabProps {
  ...
  loading: boolean
  // La firma real de `cargarClustering` (ver arriba, `AnalisisView`) es
  // `(tipo = 'kmeans') => {...}` — toma `nClusters` de `clusterCount` por
  // clausura, no como segundo argumento. Se deja opcional aquí para reflejar
  // fielmente los call sites de este archivo (`onGenerate('kmeans')`,
  // `onGenerate('jerarquico')`), en vez de forzar un segundo argumento que
  // nunca se pasa.
...
// `data` se declara como `Record<string, unknown> | null` (Task description)
// porque es el resultado crudo de un POST; esta interfaz local documenta los
// campos concretos que el JSX de abajo efectivamente lee.
interface ClusteringResult {
  ...
  pca_2d?: { x: number[]; y: number[]; variance_explained: number }
