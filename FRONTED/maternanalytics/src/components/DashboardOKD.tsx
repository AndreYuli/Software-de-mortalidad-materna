...
const ALIAS_COLUMNAS: Record<'morbilidad' | 'mortalidad', Record<string, string[]>> = {
  morbilidad: {
...
type TipoEvento = 'morbilidad' | 'mortalidad'
...
type ActiveView = 'analisis' | 'mortalidad' | 'morbilidad'
...
function buildAliasMap(requiredColumns: string[], tipo: TipoEvento): Map<string, string> {
...
function findHeaderRow(jsonData: unknown[][], requiredColumns: string[], tipo: TipoEvento): number {
...
function findBestSheet(workbook: XLSX.WorkBook, requiredColumns: string[], tipo: TipoEvento): string {
...
function UploadCard({ onFile, file, error, validating, onRemove, eventLabel }: UploadCardProps) {
  ...
  const handleZoneClick = (e: MouseEvent<HTMLDivElement>) => {
    if ((e.target as HTMLElement).closest('[data-prevent-open="true"]')) return
    openFileDialog()
  }
...
function getCie10Description(code: unknown): string {
...
function getClusterColor(clusterId: number | string): string {
  const id = parseInt(String(clusterId));
...
interface AnalisisCompleto {
  ...
  causas_cie10?: {
    top_causas?: { codigo: string; casos: number }[]
  }
...
function AnalysisHomeSection({
  ...
  const renderTrend = (compareResult: { cur: number; prev: number } | null) => {
  ...
  const getMonthlyData = (dist: Record<string, Record<string, number>> | undefined, targetYear: string) => {
  ...
  // Line Chart Config
  const lineChartData: Record<string, unknown>[] = [];
...
export default function DashboardOKD({ onLogout }: DashboardOKDProps) {
  const [activeView, setActiveView] = useState<ActiveView>('analisis')
  ...
  const [mortalidadFile, setMortalidadFile] = useState<File | null>(null)
  const [morbilidadFile, setMorbilidadFile] = useState<File | null>(null)
  const [mortalidadError, setMortalidadError] = useState<FileValidationError | null>(null)
  const [morbilidadError, setMorbilidadError] = useState<FileValidationError | null>(null)
  ...
  const [mortalidadAnalyzeError, setMortalidadAnalyzeError] = useState<string | null>(null)
  const [morbilidadAnalyzeError, setMorbilidadAnalyzeError] = useState<string | null>(null)
  ...
  // Manejo de archivos
  const handleMortalidadFile = async (file: File | null) => {
    if (!file) {
  ...
  const handleMorbilidadFile = async (file: File | null) => {
    if (!file) {
  ...
  const handleSelectAnalysisType = (tipo: string) => {
    setAnalysisType(tipo)
    setActiveView('analisis')
  ...
  const handleEventosChange = (nextEventos: string[]) => {
  ...
  // Nota (migración TS): filterEventos, currentAnalysisId, handleSelectAnalysisType,
  // handleEventosChange y pageTitle ya estaban calculados pero sin consumidor en el JSX
  // en el archivo .jsx original. Se preservan sin cambios de lógica; estas referencias
  // solo satisfacen noUnusedLocals sin alterar comportamiento.
  void filterEventos
  void currentAnalysisId
  void handleSelectAnalysisType
  void handleEventosChange
  void pageTitle
...
// Nota (migración TS): CasosCombinados y FilterPanel ya estaban definidos pero sin
// ningún <CasosCombinados/> ni <FilterPanel/> en el JSX del archivo .jsx original.
// Se preservan sin cambios de lógica; estas referencias solo satisfacen
// noUnusedLocals sin alterar comportamiento.
void CasosCombinados
void FilterPanel
